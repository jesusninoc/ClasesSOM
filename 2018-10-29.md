...
ls -Recurse
```
- Listar todos los ficheros, pero sólo el 
```PowerShell
ls -Name 
```
...
```PowerShell
 ni -ItemType directory usuario24,usuario25
 ```
- Crear un fichero para cada usuario
```PowerShell
Set-Location(sl) .\usuario24  ----  cd .\usuario24    #Entre parentesis van los alias para que el script sea mucho mas corto. 
New-Item fichero26.txt -Value "usuario 24"
```
- Añadir contenido a cada fichero creado para cada usuario
```PowerShell
New-Item -Path .\usuario24 -Name fichero28.txt -ItemType file -Force -Value "pepe"
# Este y el New-Item de arriba es los mismo pero de diferente forma.
```
- Eliminar un directorio
```PowerShell
rm .\usuario1 # Aqui poner el directorio que cada uno quiera eliminar. 
```
- Renombrar un directorio
```PowerShell
Rename-Item .\usuario1 # El directorio que cada uno quiera renombrar. 
```
---------------------
